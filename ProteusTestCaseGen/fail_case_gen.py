# -*- coding: utf-8 -*-
"""proteus-test-gen.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/173VHk4UmahmBaxz-Hduo7H8VtUcys8u-
"""
...
# This one string will be the entire code
code = ""
...
# This list will hold all the varaible syntax
variables = []

# This will hold all the states syntax
states = []

# This will hold all the events syntax
events = []

# This will hold all the actor syntax
actors = []
...
# Type: 'actorname' | 'statename' | 'eventname'
def define_proteus_varaibles(actorname=BLANK, statename=BLANK, eventname=BLANK):
  None # at the moment
...
def define_func(FormalFuncArgs=BLANK, Block=BLANK):
  func_code = "func " + rand_list_entry(all_function_names)
  # randomly decides if theres args
  if rand_num(0, 1):
    # Randomly pick if func is a method with args
    if rand_num(0, 1):  # Is method
      func_code += "(" + FormalFuncArgs + ")" + " -> " + rand_list_entry(all_var_types)
      func_code += Block
    else: # Nod method
      func_code += "(" + FormalFuncArgs + ")"
      func_code += Block
  else:
    # Randomly pick if func is a method without args
    if rand_num(0, 1):  # Is method
      func_code += "()" + " -> " + rand_list_entry(all_var_types)
      func_code += Block
    else: # Not method
      func_code += "()"
      func_code += Block
...
def event_def_on(OnBody=BLANK):
  event_code = "on " + rand_list_entry(all_event_names)
  event_code += none_checker(OnBody)
  return event_code


# DefEntry: 'entry' { Block }
def event_def_entry(Block=BLANK):
  event_code = "entry " + rand_list_entry(all_event_names)
  event_code += none_checker(Block)
  return event_code


# DefExit: 'exit' { Block }
def event_def_exit(Block=BLANK):
  event_code = "exit " + rand_list_entry(all_event_names)
  event_code += none_checker(Block)
  return event_code
...
def initial_state():
  return f"initial {rand_list_entry(all_state_names)};\n"
...
def define_state(StateItem=BLANK):
    ...
    state_code = "state " + state_name + " {" + f"// start of {state_name}\n"  # Initialize state_code here
    state_code += none_checker(StateItem)
    state_code += "}" + f"// end of {state_name}\n"
...
def define_statemachine(StateItem=BLANK):
  statemachine_code = "\nstatemachine {\n"
  statemachine_code += none_checker(StateItem)
  statemachine_code += "}\n"
  return statemachine_code


# DefActor: 'actor' ActorName '{' ActorItem* '}'
def define_actor(ActorItem=BLANK):
  actor_code = f"actor {rand_list_entry(all_actor_names)} " + "{\n"
  actor_code += none_checker(ActorItem)
  actor_code += "}\n"
  return actor_code
...
def define_expr_list_paren(Expr):
  expr_code = "("
  expr_code += none_checker(', '.join(Expr))
  expr_code += ")"
  return expr_code
...
def define_println(ExprListParen=BLANK):
  println_code = "println "
  println_code += none_checker(ExprListParen)
  println_code += ";\n"
  return println_code
...
def just_go_stmt(StateName=BLANK, jgs_Block=BLANK):
  jgs_code = "go " + none_checker(StateName)
  jgs_code += none_checker(jgs_Block)
  return jgs_code


# ElseGoStmt: 'go' StateName Block
def else_go_stmt(StateName=BLANK, egs_Block=BLANK):
  egs_code = "go " + none_checker(StateName)
  egs_code += none_checker(egs_Block)
  return egs_code
...
def generate_fail_case():
  ...
  for _ in range(rand_num(1, 5)):
    ...
    function = "\n" # define_func(make_args(), define_block())
